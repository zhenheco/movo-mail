...
| Canonical cf-mail `/send` | ✅ | accepts the versioned arrays, max 32 relay attachments, idempotent replay/mismatch, custom threading headers, provider field mapping, and rejects malformed recipient/attachment input | upstream `test/send.test.ts` / `test/idempotency.test.ts` |
...
- **Acceptance criteria**:
  ...
  - [ ] Movo accepts 10 attachment entries and rejects 11 before relay; upgraded cf-mail accepts 32 entries and rejects 33, rejects malformed values, and enforces the documented total-message limit.
