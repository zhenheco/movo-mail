...
Movo POST /api/send accepts `contract_version: "movo-send-v1"`, `from` (optional for backward compatibility and never authoritative), `to`, optional `cc` and `bcc`, `subject`, `text` or `html`, optional `attachments`, `headers` (client threading headers ignored), `threadId`, `mailboxId`, `idempotencyKey`, `replyMode`, `bccProvenance`, and `bccConfirmation`. EmailAddress is `{address:string,name?:string}`; addresses use the existing bare-address grammar, are trimmed, and invalid tokens are errors rather than silently dropped. `from` is always server-derived from the authenticated mailbox.

An attachment is {filename:string,contentType:string,contentBase64:string,contentId?:string,inline?:boolean}. Filename and contentType are non-blank, contentBase64 is standard base64 decoding to at least one byte, and an 11-file picker batch is atomic.

Success is HTTP 200 `{ok:true,id:string,status:"queued"|"pending"|"sent"|"sent_unarchived",messageId:string|null}`. Validation is HTTP 400 `{error:string,code:string}`; stable codes are `invalid_recipient`, `recipient_limit`, `invalid_attachment`, `attachment_limit`, `message_size_limit`, `missing_body`, `invalid_thread`, `reply_all_no_recipients`, and `reply_all_bcc_confirmation_required`. Authorization is 403 `mailbox_forbidden`, suppression is 422, idempotency mismatch is 409 `idempotency_mismatch`, and upstream or network failure is 502. The relay accepts the same recipient/body/attachment fields plus the `Idempotency-Key` header and returns `{id:string,status:"pending"|"sent"|"failed"}` or `{error:{code:string,message:string}}`. Error precedence is JSON shape, recipient grammar/count, attachment grammar/count, Base64 decode/non-zero, attachment Base64 character total, complete serialized message bytes, mailbox/thread authorization, idempotency claim, then relay submission; the first failing rule wins.
...
The client creates one key per logical send and retains it across retries. The adapter sends it in `Idempotency-Key`. The relay scopes it to the API key and retains it for 24 hours. Same key and byte-equivalent normalized payload returns the same id and status with one provider call and one delivery. Changed recipient, body, header, or attachment returns HTTP 409 `idempotency_mismatch` with no provider call. Movo's durable `send_attempts` row is unique by mailbox and key and is the local authority for `queued`, `pending`, `sent`, `failed`, and `sent_unarchived` transitions.

Concurrent same-key requests produce one call and one delivery with the same provider id. Timeout before provider acceptance returns 502 and same-key retry is safe. If provider acceptance occurs but the response is lost, replay or status polling returns the same provider id and exactly one complete sent copy or an explicit sent_unarchived state. Delivery success with D1 or R2 failure never reports complete archival success and cannot send again.
...
The immutable release evidence records UTC timestamp, upstream commit, Movo commit, endpoint, auth state, health response, compatibility request and response with redacted credentials, version-skew result, rollback target, and a no-send oracle. Verification commands and exit codes are recorded with stdout and stderr hashes. Destructive QA uses isolated fixtures only.
